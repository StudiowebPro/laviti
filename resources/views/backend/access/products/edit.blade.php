...
@section('content')
    {{ Form::model($product, ['route' => ['admin.access.user.update', $product], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'PATCH']) }}
        ...
        <div class="box-body">
            ...
            @if ($product->id != 1)
                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-1">
                        {{ Form::checkbox('confirmed', '1', $product->confirmed == 1) }}
                    </div><!--col-lg-1-->
                </div><!--form control-->
                    ...
                    <div class="col-lg-3">
                        @if (count($roles) > 0)
                            @foreach($roles as $role)
                                <input type="checkbox" value="{{$role->id}}" name="assignees_roles[{{ $role->id }}]" {{ is_array(old('assignees_roles')) ? (in_array($role->id, old('assignees_roles')) ? 'checked' : '') : (in_array($role->id, $product_roles) ? 'checked' : '') }} id="role-{{$role->id}}" /> <label for="role-{{$role->id}}">{{ $role->name }}</label>
